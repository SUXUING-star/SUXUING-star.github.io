...
class EnhancedBERTClassifier(nn.Module):
    def __init__(self, dropout=0.5):  # 降低dropout率
...
def train_model(model, train_loader, val_loader, epochs=10):
    ...
    optimizer = torch.optim.AdamW([
        ...
        {'params': model.multihead_attn.parameters(), 'lr': 1e-4},
        {'params': model.feature_enhancement.parameters(), 'lr': 1e-4},
        {'params': model.classifier.parameters(), 'lr': 1e-4}
    ], weight_decay=0.01)
    
    # 调整warmup策略
    num_training_steps = len(train_loader) * epochs
    num_warmup_steps = num_training_steps // 10  # 减少warmup步数
